...
import { unsubscribeFromNewsletter } from '@/app/unsubscribe/actions'

type UIState = 'checking' | 'authenticated' | 'confirming' | 'loading' | 'success' | 'error'

interface UnsubscribeFormProps {
  dict: UnsubscribeNamespace
}

export default function UnsubscribeForm({ dict }: UnsubscribeFormProps) {
    ...
    <>
        ...
        <div className="w-full max-w-sm">
          ...
          {uiState === 'checking' && (
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              ...
              <p className="text-sm text-[var(--text-3)]">{dict.common.loading}</p>
            </div>
          )}
      ...
      <AnimatePresence>
        {(uiState === 'confirming' || uiState === 'loading') && (
          <ConfirmDialog
            title={dict.confirmDialog.title}
            message={`${dict.confirmDialog.message}`}
            confirmLabel={dict.confirmDialog.confirmLabel}
