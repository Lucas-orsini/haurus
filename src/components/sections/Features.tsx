...
import { BarChart3, Activity, Target, TrendingUp, Zap, Clock } from 'lucide-react'
import { cn } from '@/lib/utils'

// ── Variants (type-safe, no string ease) ────────────────────────────────────
...
function AnimatedCounter({ target, suffix = '' }: { target: number; suffix?: string }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      setCount(Math.min(target * p, target))
...
function CircularProgress({ value, label, color }: { value: number; label: string; color: string }) {
  ...
  const dashOffset = animated ? circumference * (1 - value / 100) : circumference
...
function MiniBarChart() {
  const heights = ['45%', '68%', '52%', '80%', '60%', '72%', '88%']
...
function BentoCard({
  ...
  return (
    <motion.div
      ...
      className={cn(
        'relative h-full min-h-[220px] rounded-xl border border-[var(--border-md)] bg-[var(--surface-1)] p-5 flex flex-col gap-2 overflow-hidden group',
        className
      )}
    >
      {/* Hover gradient */}
      ...
      <div className="max-w-5xl mx-auto mb-16 text-center">
        ...
        >
          ...
          <h2 className="text-3xl md:text-4xl font-medium tracking-tight text-[var(--text-1)] mb-4">
            The edge professionals use
          </h2>
          <p className="text-[var(--text-2)] max-w-xl mx-auto text-sm leading-relaxed">
            Eight data dimensions that bookmakers price into every ATP match. You get them all — updated after each match.
          </p>
        </motion.div>
      </div>
        ...
        >
          ...
          <BentoCard>
            ...
            <div className="mt-auto flex flex-col items-center justify-center pt-3">
              ...
              <p className="text-[10px] text-[var(--text-3)] mt-1 text-center">
                pressure score · avg over last 20 matches
              </p>
            </div>
          </BentoCard>
          ...
          <BentoCard>
            ...
            <div className="mt-auto pt-3">
              <div className="flex items-center justify-between mb-1.5 px-1">
                ...
                <span className="flex items-center gap-0.5 text-xs font-semibold text-[var(--green)]">
                  +38 <TrendingUp size={10} />
                </span>
              </div>
              <div className="h-12 rounded border border-[var(--border-md)] bg-[var(--surface-2)] flex items-center justify-center">
                <Zap size={16} className="text-[var(--green)] opacity-40" strokeWidth={1.5} />
              </div>
            </div>
          </BentoCard>
          ...
          <BentoCard>
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-sm font-semibold text-[var(--text-1)] tracking-tight">Match Volume</h3>
                <p className="text-[11px] text-[var(--text-3)]">Data coverage · ATP Tour</p>
              </div>
            </div>
            <div className="mt-auto flex flex-col items-center justify-center pt-3">
              <div className="text-4xl font-bold text-[var(--text-1)] font-mono">
                <AnimatedCounter target={250} suffix="k+" />
              </div>
              <p className="text-[10px] text-[var(--text-3)] mt-1 text-center">
                matches analysed across ATP Tour
              </p>
            </div>
          </BentoCard>
        </motion.div>
