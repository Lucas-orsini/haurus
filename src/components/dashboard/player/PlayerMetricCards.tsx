  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
      ...
      <div className="bg-[var(--surface-1)] border border-[var(--border-md)] rounded-lg p-4">
        <p className="text-xs text-[var(--text-3)] mb-2 uppercase tracking-wider">{t('player.metricCards.glicko')}</p>
      ...
      <div className="bg-[var(--surface-1)] border border-[var(--border-md)] rounded-lg p-4">
        <p className="text-xs text-[var(--text-3)] mb-2 uppercase tracking-wider">{t('player.metricCards.pServe')}</p>
        <p className="text-2xl font-semibold text-[var(--text-1)] tabular-nums font-mono">
          {pServePct}
        </p>
        {pServeDelta !== null ? (
          <p className={cn('text-xs mt-1 flex items-center gap-1', getDeltaColor(pServeDelta > 0 ? 1 : -1))}>
            ...
            <span>
              {pServeDelta >= 0 ? '+' : ''}{(pServeDelta * 100).toFixed(1)}% {t('player.metricCards.vsAtp')}
            </span>
          </p>
        ) : (
          <p className="text-xs text-[var(--text-3)] mt-1">{surface}</p>
      ...
      {/* Card 3 — Momentum TD */}
      <div className="bg-[var(--surface-1)] border border-[var(--border-md)] rounded-lg p-4">
        <p className="text-xs text-[var(--text-3)] mb-2 uppercase tracking-wider">{t('player.metricCards.momentum')}</p>
        <p className={cn(
        ...
        <p className="text-xs text-[var(--text-3)] mt-1">{t('player.metricCards.days30')}</p>
      </div>
    </div>
  )
}
