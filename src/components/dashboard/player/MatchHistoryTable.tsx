...
export default function MatchHistoryTable({
  ...
  const mh = dict.player?.matchHistory ?? {
    title: 'Derniers matchs',
    emptyTitle: 'Aucun match disponible',
    emptyDesc: "L'historique de ce joueur est vide",
    columns: {
      date: 'Date',
      opponent: 'Adversaire',
      tournament: 'Tournoi',
    ...
    return (
      <div className="bg-[var(--surface-1)] border border-[var(--border-md)] rounded-lg p-8">
        <div className="flex flex-col items-center justify-center text-center">
          ...
          <p className="text-sm text-[var(--text-2)]">{mh.emptyTitle}</p>
          <p className="text-xs text-[var(--text-3)] mt-1">{mh.emptyDesc}</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="bg-[var(--surface-1)] border border-[var(--border-md)] rounded-lg overflow-hidden">
      <div className="px-4 py-3 border-b border-[var(--border-md)]">
        <h3 className="text-sm font-medium text-[var(--text-1)]">{mh.title}</h3>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="border-b border-[var(--border-md)]">
              <th className="hidden md:table-cell px-4 py-2.5 text-left text-xs font-medium text-[var(--text-3)] uppercase tracking-wide">{mh.columns.date}</th>
              <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--text-3)] uppercase tracking-wide">{mh.columns.opponent}</th>
              <th className="hidden md:table-cell px-4 py-2.5 text-left text-xs font-medium text-[var(--text-3)] uppercase tracking-wide">{mh.columns.tournament}</th>
              <th className="hidden md:table-cell px-4 py-2.5 text-left text-xs font-medium text-[var(--text-3)] uppercase tracking-wide">{mh.columns.surface}</th>
              <th className="hidden md:table-cell px-4 py-2.5 text-left text-xs font-medium text-[var(--text-3)] uppercase tracking-wide">{mh.columns.score}</th>
              <th className="px-4 py-2.5 text-center text-xs font-medium text-[var(--text-3)] uppercase tracking-wide">{mh.columns.result}</th>
              <th className="px-4 py-2.5 text-center text-xs font-medium text-[var(--text-3)] uppercase tracking-wide w-24">{mh.columns.metrics}</th>
            </tr>
          </thead>
          <tbody>
