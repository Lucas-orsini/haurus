...
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | Yes | Supabase Dashboard → **Project Settings** → **API** → copy **anon/public** key | Public key for client-side Supabase access (safe with RLS) |
...
| `TELEGRAM_BOT_TOKEN` | Yes | Open Telegram, chat with [@BotFather](https://t.me/botfather), send `/newbot`, follow prompts, copy the token | Telegram bot API token |
| `TELEGRAM_BOT_SECRET` | Yes | Set this yourself when calling `setWebhook` with the secret parameter | Secret string for HMAC-SHA256 webhook verification |
...
- `FAIL` — Something broke ❌ — look at the error message below for what failed
