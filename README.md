...
| `SUPABASE_SERVICE_ROLE_KEY` | Yes | Supabase Dashboard → **Project Settings** → **API** → copy **service_role** key | Server-side key that bypasses RLS — never expose to browser |
| `TELEGRAM_BOT_TOKEN` | Yes | Open Telegram → search **@BotFather** → send `/newbot` → follow prompts → copy the token | Your bot's API token |
| `TELEGRAM_BOT_SECRET` | Yes | Choose any random string (e.g. use a password generator) | Secret for HMAC-SHA256 webhook verification |

## 🧪 Running Tests

Unit tests automatically check that your code does what it's supposed to do — no manual clicking required.
...
Watch mode — re-runs tests automatically when a file changes:
...
- `FAIL` — something broke, see the error message below for what failed

Tests included:
- `__tests__/auth-validators.test.ts` — validates auth input fields
- `__tests__/auth.test.ts` — tests authentication logic
- `__tests__/dashboard/formatMetric.test.ts` — tests metric formatting utilities
...
3. In **Environment Variables**, add every variable from your `.env.local`:
   - `NEXT_PUBLIC_SUPABASE_URL`
   - `NEXT_PUBLIC_SUPABASE_ANON_KEY`
   - `SUPABASE_SERVICE_ROLE_KEY`
   - `TELEGRAM_BOT_TOKEN`
...
Vercel will automatically install dependencies and deploy your app. Your site will be live at a `vercel.app` URL.
